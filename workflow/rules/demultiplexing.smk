...
def return_renamed_fastqs(wildcards):
    ...
    return [f"{config['projdir']}/guppy_basecalling_{config['accuracymode']}/demultiplexing/{barcode}/{barcode}.fastq.gz" for barcode in barcodes]
...
rule guppy_demultiplexing:
    ...
    output:
        "{projdir}/guppy_basecalling_{accuracymode}/demultiplexing/barcoding_summary.txt"
    message: "Demultiplex output with Guppy"
    params:
        guppydir = config['guppydir'],
        dir = "{projdir}/guppy_basecalling_{accuracymode}/guppy/pass",
        outdir = "{projdir}/guppy_basecalling_{accuracymode}/demultiplexing",
...
checkpoint summarise_expected_barcodes:
    input:
        "{projdir}/guppy_basecalling_{accuracymode}/demultiplexing/barcoding_summary.txt"
    output:
        "{projdir}/guppy_basecalling_{accuracymode}/demultiplexing/expected_barcodes.txt"
...
rule concat_fastq:
    ...
    params:
        fastq = lambda wildcards: " ".join(glob(f"{config['projdir']}/guppy_basecalling_{config['accuracymode']}/demultiplexing/{wildcards.barcode}/*.fastq.gz")),
        n_fastq = lambda wildcards: len(glob(f"{config['projdir']}/guppy_basecalling_{config['accuracymode']}/demultiplexing/{wildcards.barcode}/*.fastq.gz")) 
    shell:
        """
